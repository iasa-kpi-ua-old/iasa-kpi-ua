...
reply.prototype._oneMatchString = function(id, match, meta){
  ...
  if(meta.pathname.indexOf(match, 0)==0){
    return this._oneMatchReturn(match, match.length/meta.pathname.length);
  }
  else if(match=='*'){
    return this._oneMatchReturn(meta.pathname, 0);
  }
  else if(RE.test(match)){
    var m = RE.exec(match);
    return this._oneMatchObject(id, { method: m[2], host: m[3], path: m[4], priority: Number(m[1]) }, meta);
  }
  return this._oneMatchReturn('', 0);
};
